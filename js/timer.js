...
function counterPt(foo) {
  ...
  timer.innerHTML = `<span class="hh">${
    th == "" ? "00" : th
  }</span>:<span class="mm">${tm == "" ? "00" : tm}</span>:<span class="ss">${
    ts == "" ? "00" : ts
  }</span>`;
...
set_time.addEventListener("click", () => {
  ...
  time_txt.innerText = `Timer ( ${lh != 0 ? lh + "Hr" : ""} ${
    lm != 0 ? lm + "Min" : ""
  } ${ls != 0 ? ls + "s" : ""} )`;
});
...
sub_q.addEventListener("click", () => {
  const name = document.querySelector(".str-name");
  localStorage.setItem("name-db", name.value);
  let type_check = document.querySelector(".user_wr input").value;
