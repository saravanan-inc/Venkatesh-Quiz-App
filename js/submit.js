  ...
  static showCorrect(dummie) {
    ...
    function sumt() {
      ...
      qtn_ans_arr.forEach((items, target) => {
        [
          ...qtn_ans_arr[target].firstElementChild.nextElementSibling.children,
        ].forEach((items) => {
      ...
      // check ans for right.
      qtn_ans_arr.forEach((items, target) => {
        [...items.firstElementChild.nextElementSibling.children].forEach(
          (items, pos) => {
      ...
      if (clk_count == qtn.length) {
        ...
        backenddb(total, crct_ans, percentage);
  ...
  static rankOne(type) {
    ...
    switch (type) {
      case "obo":
        ...
        pr_rply1.innerHTML = `<button class='q_prev replay b_2px_bg_b'>Replay</button><button id="sel_que_obo_prev" class='q_prev b_2px_bg_b' onclick="QtnNav.prevQtn()">Prev</button><button onclick="seeRankPage()" class='q_next  b_2px_bg_b'>See Your Rank</button>`;
        break;
      case "all":
        let pr_rply2 = [...pr_rply][pr_rply.length - 1];
        pr_rply2.innerHTML = `<button class='q_prev replay b_2px_bg_b'>Replay</button><button class='q_next  b_2px_bg_b' onclick="seeRankPage()">See Your Rank</button>`;
        break;
      default:
        break;
    }
...
function showByAll() {
  ...
  s_all.forEach((items, pos) => {
    if (pos == s_all.length - 1) {
      items.lastElementChild.previousElementSibling.innerHTML = `<button class='q_prev sel_que_obo replay b_2px_bg_b'>Replay</button><button onclick="seeRankPage()" class='q_next  b_2px_bg_b'>See Your Rank</button>`;
    } else {
...
// document.getElementById("name-db").onchange(() => {
//   console.log("h");
// });
...
async function backenddb(total, crct_ans, percentage) {
  const ref = collection(db, "QuizApp-Ranks");
  const name = localStorage.getItem("name-db");
  const docRef = await addDoc(ref, {
    Name: name,
    Percentage: percentage,
    Correct_Answer: crct_ans,
